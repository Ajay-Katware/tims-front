...
export class CreateInvoiceComponent implements OnInit {
  ...
  getFullSOInformation(id:number){
    this.salesOrdersService.getSalesOrder(id).subscribe(data=>{
      ...
      if(this.salesOrder.invoiceno!=null){
        this.getNextInvoice();
      }
    })
  }
  ...
  getNextInvoice(): void {
    this.salesOrdersService.getNextInvoiceNumber().subscribe(result => {
      this.autonumber = result;
      this.invoiceno = this.autonumber.nextno;
      this.setValue();
    })
  }
  ...
  confirmAdd(): void {
    this.salesOrdersService.createInvoice(this.id, this.invoiceno).subscribe(data=>{
      this.showLoader();
      if(!data){
        this.toasterService.openErrorSnackBar('SomethingWrong','', 3000);
      }else{
        this.toasterService.openSuccessSnackBar('Created Successfully','ok', 3000);
      }
    })
